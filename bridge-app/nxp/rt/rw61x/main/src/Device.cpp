...
DeviceDimmable::DeviceDimmable(const char * szDeviceName, std::string szLocation) : Device(szDeviceName, szLocation)
{
}
...
void DeviceOnOff::SetLevelValue(uint8_t level)
{
    ChipLogProgress(DeviceLayer, "OnOffDevice[%s]: New Level=\"%d\"", mName, level);
...
DeviceColor::DeviceColor(const char * szDeviceName, std::string szLocation) : Device(szDeviceName, szLocation)
{
}
void DeviceColor::SetChangeCallback(DeviceCallback_fn aChanged_CB)
{
    mChanged_CB = aChanged_CB;
}

void DeviceColor::HandleDeviceChange(Device * device, Device::Changed_t changeMask)
{
    if (mChanged_CB)
    {
        mChanged_CB(this, (DeviceColor::Changed_t) changeMask);
    }
}
