...
public:
    ...
    void SetLevelValue(uint8_t level);
...
class DeviceDimmable : public Device
{
public:
    enum Changed_t
    {
        kChanged_Dimmable = kChanged_Last << 1,
    } Changed;
...
class DeviceColor : public Device
{
public:
    enum Changed_t
    {
        kChanged_Color = kChanged_Last << 1,
    } Changed;

    DeviceColor(const char * szDeviceName, std::string szLocation);
