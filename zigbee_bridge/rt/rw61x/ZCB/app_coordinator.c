...
void vAppHandleZdoEvents( BDB_tsZpsAfEvent *psZpsAfEvent)
{
    ...
    switch(psAfEvent->eType)
    {
        case ZPS_EVENT_APS_DATA_INDICATION:
            DBG_vPrintf(TRUE, "APP-ZDO: Data Indication Status %02x from %04x Src Ep %d Dst Ep %d Profile %04x Cluster %04x\r\n",
            ...
            zps_bAplZdpUnpackResponse (psAfEvent, &sApsZdpEvent);

            switch(sApsZdpEvent.u16ClusterId)
            {
                case ZPS_ZDP_DEVICE_ANNCE_REQ_CLUSTER_ID:
                {
                    ZCB_HandleDeviceAnnounce(sApsZdpEvent.uZdpData.sDeviceAnnce.u16NwkAddr, 
                                                sApsZdpEvent.uZdpData.sDeviceAnnce.u64IeeeAddr, 
                                                sApsZdpEvent.uZdpData.sDeviceAnnce.u8Capability);

                    // uint8 u8Index = 0xff;
                    // if ( bIsMacAddrInMacInstallCodeTable(sApsZdpEvent.uZdpData.sDeviceAnnce.u64IeeeAddr, &u8Index))
                    // {
                    //     vDeleteMacAddrFromMacInstallCodeTable(sApsZdpEvent.uZdpData.sDeviceAnnce.u64IeeeAddr, u8Index);
                    // }
                    break;
                }
                case ZPS_ZDP_SIMPLE_DESC_RSP_CLUSTER_ID:
                {
                    ...
                    {
                        ZCB_HandleSimpleDescriptorResponse(sApsZdpEvent.uZdpData.sSimpleDescRsp.u16NwkAddrOfInterest, 
                                                           sApsZdpEvent.uZdpData.sSimpleDescRsp.sSimpleDescriptor.u8Endpoint,
                                                           sApsZdpEvent.uZdpData.sSimpleDescRsp.sSimpleDescriptor.u16ApplicationProfileId,
                                                           sApsZdpEvent.uZdpData.sSimpleDescRsp.sSimpleDescriptor.u16DeviceId,
                                                           sApsZdpEvent.uZdpData.sSimpleDescRsp.sSimpleDescriptor.uBitUnion.u8Value,
                                                           sApsZdpEvent.uZdpData.sSimpleDescRsp.sSimpleDescriptor.u8InClusterCount,
                                                           sApsZdpEvent.uZdpData.sSimpleDescRsp.sSimpleDescriptor.u8OutClusterCount,
                                                           sApsZdpEvent.uLists.au16Data);
                    }
                    break;
                }
                case ZPS_ZDP_IEEE_ADDR_RSP_CLUSTER_ID:
                {
                    ZCB_HandleIeeeAddressReponse(sApsZdpEvent.uZdpData.sIeeeAddrRsp.u64IeeeAddrRemoteDev,
                                                 sApsZdpEvent.uZdpData.sIeeeAddrRsp.u16NwkAddrRemoteDev);
                    break;
                }
                case ZPS_ZDP_NWK_ADDR_RSP_CLUSTER_ID:
                {
                    ZCB_HandleNetworkAddressReponse(sApsZdpEvent.uZdpData.sNwkAddrRsp.u64IeeeAddrRemoteDev,
                                                    sApsZdpEvent.uZdpData.sNwkAddrRsp.u16NwkAddrRemoteDev);
                    break;
                }
                case ZPS_ZDP_ACTIVE_EP_RSP_CLUSTER_ID:
                {
                    ZCB_HandleActiveEndPointResp(sApsZdpEvent.uZdpData.sActiveEpRsp.u16NwkAddrOfInterest,
                                                 sApsZdpEvent.uZdpData.sActiveEpRsp.u8ActiveEpCount,
                                                 sApsZdpEvent.uLists.au8Data);
                    break;
                }
        ...
        case ZPS_EVENT_NWK_LEAVE_INDICATION:
            DBG_vPrintf(TRACE_APP, "APP-ZDO: Leave Indication %016llx Rejoin %d\r\n",
                    ...
                    ZCB_HandleDeviceLeave(psAfEvent->uEvent.sNwkLeaveIndicationEvent.u64ExtAddr, psAfEvent->uEvent.sNwkLeaveIndicationEvent.u8Rejoin);
            break;
